...
def initialize (context):
	"""
	Called once at the start of the program. One-time startup logic goes here.
	"""
	...
	# Re-balance on the first trading day of each week at 11AM.
...
def make_pipeline (context):
	...
	used to rank stocks based on different factors, including built-in factors,
	...
	# Create a Returns factor with a 5-day look back window for all
	# securities in our QTradableStocksUS Filter.
	recent_returns = Returns(window_length=context.returns_lookback, mask=universe)
	...
	pipe = Pipeline(
		columns={'low_returns': low_returns, 'high_returns': high_returns},
...
def rebalance (context, data):
	"""
	This re-balancing function is called according to our schedule_function settings.
	"""
...
def record_vars (context, data):
	...
	# leverage is plotted.
	algo.record(leverage=context.account.leverage, long_count=longs, short_count=shorts)
